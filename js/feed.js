// Gulf Coast Radar Feed Page JavaScript
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Auto-refresh every 60 seconds (simulated)
  setInterval(() => {
    refreshFeed();
  }, 60000);
});
...
function displayPosts(posts) {
  ...
  if (posts.length === 0) {
    container.innerHTML = `
      <div class="feed-empty-state">
        ...
        <p class="feed-empty-state-description">Try adjusting your filters to see more posts</p>
      </div>
    `;
    return;
  }
  ...
  posts.forEach(post => {
    const business = (typeof allBusinesses !== 'undefined' && allBusinesses)
      ? allBusinesses.find(b => b.id === post.businessId)
      : null;
    ...
    postsHtml += `
      <div class="social-post-card" onclick="window.location.href='profile.html?id=${post.businessId}'">
...
function refreshFeed() {
  // In a real implementation, this would fetch new posts from the server
  // For now, we'll just re-display the existing posts
  console.log('Feed refreshed at:', new Date().toLocaleTimeString());
  allPosts = getAllPosts();
  displayPosts(allPosts);
}
